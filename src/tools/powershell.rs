...
impl Tool for PowerShellTool {
    ...
    async fn execute(&self, args: Value) -> ToolResult {
        ...
        if interactive {
            ...
            return spawn_interactive_powershell(
                pwsh_args,
                command,
                wait_seconds,
                forced_interactive,
        ...
        if background {
            ...
            return spawn_background_powershell(cmd, command, wait_seconds, persistent, ttl_secs).await;
        }
...
async fn spawn_background_powershell(
    ...
    match result {
        Ok(outcome) => {
            let mut msg = format!("Started PowerShell job #{} ({})", outcome.id, job_mode_label(outcome.persistent, outcome.ttl_secs));
...
async fn spawn_interactive_powershell(
    ...
    match result {
        Ok(outcome) => {
            ...
            msg.push_str(&format!(
                "Started interactive PowerShell job #{} ({})\n",
                outcome.id,
