...
impl Tool for PowerShellTool {
        ...
        ToolDefinition {
            ...
            parameters: json!({
                ...
                "properties": {
                    ...
                    "interactive": {
                        "type": "boolean",
                        "description": "If true, run in a PTY so interactive TUI menus and prompts work. Returns a process id; drive it with shell_input. Implies detached. Use for npm create vite, npm init, gh auth login, or any command that opens a selector/REPL."
                    },
                    "wait_seconds": {
                        "type": "number",
    ...
    async fn execute(&self, args: Value) -> ToolResult {
        ...
        if interactive {
            ...
            return spawn_interactive_powershell(pwsh_args, command, wait_seconds).await;
        }

        if background {
            let mut cmd = Command::new("powershell");
            cmd.arg("-NoProfile").arg("-Command").arg(command);
            return spawn_background_powershell(cmd, command, wait_seconds).await;
        }
...
async fn spawn_background_powershell(
    cmd: Command,
    command_str: &str,
    wait_seconds: f64,
) -> ToolResult {
    match background::spawn_background(cmd, command_str.to_string(), "powershell".to_string(), wait_seconds).await {
        Ok(outcome) => {
            let mut msg = format!(
                "[Background] Process started. id={} shell=powershell\nCommand: {}\nStatus: {}\nInitial output (captured {}s):\n",
...
async fn spawn_interactive_powershell(
    pwsh_args: Vec<String>,
    command_str: &str,
    wait_seconds: f64,
) -> ToolResult {
    ...
    .await
    {
        Ok(outcome) => {
            let mut msg = format!(
                "[Interactive PTY] Process started. id={} shell=powershell\nCommand: {}\nStatus: {}\nInitial output (captured {}s):\n",
                outcome.id, command_str, outcome.status.label(), wait_seconds
            );
            if outcome.initial_output.is_empty() {
