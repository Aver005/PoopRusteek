# .memories INDEX
> Agent entry point. If you were handed this project cold, START HERE. Last updated: 2026-08-29 (**отложенные задачи агента** — инструмент `timer` + `TimerStore` в реестре инструментов, срабатывание на тике через чистую `app::timers::route`, `/timers`, `⏰:N` в статус-баре; персистентности нет сознательно, в харнессе таймеры не стреляют. `JOURNAL/2026-08-29-timers.md`; тесты 699). Before: 2026-08-27 (**проход по дублированию** — замер детектором клонов: всего ~420 снимаемых строк, меньше 1% дерева, потому что крупный дедуп сделан 07-15. Главный вывод: ценнее не новые абстракции, а **не вызванные существующие** — `char_to_byte_pos` был написан, продублирован под другим именем и расписан руками в шести местах. Сделано: `ChatMessage::new`, `util::char_to_byte_pos` (один дом), `popup::option_row`/`draw_popup`, новый `render/chrome.rs`, `config::save_or_message` (16 копий строки → 1), `jsonrpc::take_sse_frame` (+3 теста), ещё три мелких. −277 строк, тесты 679, три пункта аудита закрыты. `JOURNAL/2026-08-27-duplication.md`). Before: 2026-08-27 (**отдельный проход по длинным функциям** — `execute` 297→78 (вынесен `run_foreground`, закрыт пункт аудита `:262`), `App::new` 166→63 (`AppState::new`, три литерала сведены к одному), тест на инвариант 11 харнесса. Два пункта плана снесены критикой — почему, записано в `JOURNAL/2026-08-27-long-functions.md`, чтобы не переоткрывать; тесты 676). Before: 2026-08-27 (**рефакторинг «упрощение», фазы 1-3** — фаза 1: общий словарь навигации по спискам `src/app/list.rs` вместо семи самодельных, три дефекта прокрутки закрыты. Фаза 2: `src/app/reduce.rs` — одно применение агентских событий вместо трёх разошедшихся (фокус / фон / харнесс), `AppEvent` 41→29 вариантов + вложенный `AgentEvent`, `handle_event` 313→~120, очередь запросов к человеку в `src/app/interaction.rs`. Фаза 3: `run_agent_loop` 470→145 строк — телеметрия за фасадом `agent/trace.rs`, повторы в `agent/retry.rs`, вызовы инструментов в `agent/tools_step.rs`, общий `StreamOutcome::verdict()`; попутно закрыт баг суб-агента, отдававшего пустой ответ как успешный. `JOURNAL/2026-08-27.md`; тесты 671). Before: 2026-08-26 (review fix batch → `.docs/review-2026-08-26-rust.md` (20 findings, 7 closed, 1 refuted) + vision Stage 0 → `.docs/vision-local-image-understanding.md` + context-compaction plan decided, not built → `.docs/context-compaction.md`; `JOURNAL/2026-08-26.md`; tests 504). Before: 2026-08-25 (headless harness + Docker sandbox → `reference/HARNESS.md`, `JOURNAL/2026-08-25.md`; tests 474). Before: 2026-07-15 (quality/structure audit → `reference/AUDIT-2026-07-15-QUALITY.md`; BUGS.md refreshed (+8 entries, 2 stale corrected) — clippy 0, tests 434, MSRV 1.91)
