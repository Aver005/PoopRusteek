...
## FULL COMMAND LIST (35 commands, +2 `/cwd` aliases)
...
| `/search` | — | `<query>` | Semantic + keyword search across the indexed history of all saved sessions. Off-loop lookup (`CommandResult::SearchHistory` → spawned task → results flush via `AppEvent::AddMessage` as ui_system: date, title, role, snippet, session id, `/load` hint) | `defs/search.rs` |
