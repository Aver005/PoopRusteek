...
| `src/server/openai.rs` | OpenAI dialect: `GET /[v1/]models` (live `models.snapshot()`), `POST /[v1/]chat/completions` (16 MiB body cap); fork-per-request, DeepSeek `discard_remote_session` after every call; SSE bridge (`bridge_stream`: delta/final chunks + `data: [DONE]`, client-hangup aborts upstream); stamps `LogDetail(model → resolved)` | ~320 + tests |
