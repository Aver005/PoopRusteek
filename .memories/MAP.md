...
| `src/agent/tools_step.rs` | Вызовы инструментов одного шага: `run_tool_calls` (лимит `max_tools_per_step` с явным «Skipped:» `tool_result`), `execute_tool_call` → `ask_the_user`/`spawn_task`/`run_generic_tool`, `ToolExecContext` + `emit`, `dispatch_generic_tool` (короткий лок MCP), `tool_skip_message`, `summarize_tool_result`. Вынесено из `runner.rs` 2026-08-27 | ~300 |
| `src/agent/trace.rs` | `StepTrace` — фасад над `debug_log` для телеметрии шага. Строки байт-в-байт те же, что раньше писались в `runner.rs` вручную (инвариант 11); общий префикс `conversation=N step=M/K` был скопирован 18 раз | ~200 |
...
| `src/agent/sub_agent.rs` | `run_sub_agent` — безголовый изолированный прогон (свой форк провайдера, авто-аппрув), стрим с пустым колбэком, возвращает финальный текст. С 2026-08-27 разбирает конец стрима общим `StreamOutcome::verdict()` и повторяет пустой ответ через `RetryBudget` — раньше отдавал пустоту родителю как успех | ~170 |
