...
> Last updated: 2026-09-13 (self-update rewrite: `src/update.rs` → `src/update/` module; CI split into `checks.yml`/`build.yml`/`release.yml` + `ci.yml`; `packaging/` and `scripts/ci/` added). Before: 2026-08-27 (+ `scripts/` и `.githooks/`; 2026-08-25 — `src/harness/` и `sandbox/`; размеры приблизительные, опираться на имена, не на строки)

## ENTRY / ROOT
| File | Purpose | Lines |
|------|---------|-------|
| `src/main.rs` | CLI (`--acp`, `--debug-log`, `--version`), init order, launch TUI or ACP | 85 |
...
| `src/debug_log.rs` | Optional debug-log sink — `.dev/debug.log` in a debug build, `{data}/debug.log` in a release build | 68 |
| `src/update/` | Self-updater, was `src/update.rs`: `mod.rs` (`stable`/`dev` channels, `run()`, `platform_target`/`platform_asset`), `manifest.rs` (`Manifest`, semver `Version`), `swap.rs` (install/promote/`cleanup_stale_backup`, incl. `.old.<pid>` fallback), `install_record.rs` (Windows only — HKCU uninstall-key `DisplayVersion` sync + `PooprusteekRunning` named mutex for the Inno Setup `AppMutex`). Contract with CI: `reference/AUTO-UPDATE.md` | — |

## APP LAYER (Application) — decomposed from the old ~2.4k god-file into cohesive modules
| File | Purpose | Lines |
|------|---------|-------|
| `src/app/mod.rs` | Coordinator: `App` + `AppState`, event loop (`run_loop` — drains ≤256 events then renders once behind a dirty flag), `handle_event`, `send_focused_turn`, `FOREGROUND_CHILD_PID`, `PendingInteraction` queue, onboarding/logout/wipe, `format_size`/`format_duration_secs`/`format_tool_definition`. Re-slimmed 2026-07-04 (1659→~990) by extracting `sessions.rs` + `pickers.rs`; затем 2026-08-27 (1517→~1320) выносом `reduce.rs`/`interaction.rs` и разгрузкой `handle_event` (313→~120). Очередь `PendingInteraction` живёт в `interaction.rs`. | ~1320 |
...
| `src/app/reduce.rs` | Редьюсер `AgentEvent` → `Conversation` (2026-08-27): `apply()` — единственное место, где агентское событие меняет историю (зовут фокусный чат, фоновый и `harness/driver.rs`); `turn_end` — тот же разбор без мутации, для беседы, которой у потребителя нет; `turn_tail(end, TurnOwner) -> TurnTail` — чей хвост у завершившегося хода; `TurnOwner::classify(focused, background)` — единственное место, где два флага разбираются в имя (`Focused`/`Background`/`Parallel`) | ~290 |
| `src/app/interaction.rs` | Очередь запросов к человеку, вынесена из `mod.rs` (2026-08-27): `on_tool_approval_requested`/`on_question_requested`/`present_tool_approval`/`present_question`/`present_next_interaction`/`purge_interactions_for` — один слот на экране, остальное в FIFO | ~130 |
| `src/app/list.rs` | Общий словарь навигации по спискам (2026-08-27): `ListNav` (`from_code` — только стрелки, для списков с набором текста; `from_code_vim` — плюс `k/j/g/G`), `ListCursor` (`apply`/`clamp`/`move_to`/`window`), `ListWindow` (`anchored` — окно из одного курсора; начало окна зажато по `len - visible`, иначе после растягивания терминала под списком остаётся пустота), `page_rows` + `rows::{SERVER,PROVIDER,THEME,MATCH}`, `NAV_HINT`/`NAV_HINT_ARROWS`. Заменил `clamp_scroll`, `QuestionState::update_scroll` и `clamp_autocomplete_scroll`; читают его восемь списков и оба мастера | ~380 |
...
| `src/app/multichat.rs` | `spawn_background_agent`/`spawn_sidechat`/`spawn_sub_agent`/`stop_background`/`finish_background`/`new_conversation`/`switch_to`/`cycle_focus`/pickers + `spawn_compaction`/`on_compact_finished`. `handle_background_event` удалён 2026-08-27: применение событий у фона и фокуса теперь общее (`app/reduce.rs`) | ~510 |
| `src/app/events.rs` | Кросс-слойный словарь `AppEvent` + вложенный `AgentEvent` (13 событий одного хода) (agent variants tagged with `ConversationId`; `SpawnSubAgent`; `GoalEvaluationDone(GoalEvalOutcome)`; `McpOperationDone`; `Resize { rows }`). Состояния модалок (`Modal`, `PickerState`, `QuestionState`, `OnboardingState`, `ConfirmState`, `DeleteSessionsState`) живут в `app/view_state.rs` с 2026-07-15 и лишь ре-экспортируются отсюда | ~380 |
...
| File | Purpose | Lines |
|------|---------|-------|
| `src/agent/runner.rs` | `run_agent_loop` — оркестратор хода (470→154 строки, 2026-08-27): ступени компакции через `apply_ladder`, стрим через `stream_step`, `classify_stream` (`StreamOutcome` → ошибка провайдера / спасение вызова), `finish_or_retry` (сломанный `<tool_use>` и пустой ответ, границы держит `RetryBudget`). Исполнение вызовов делегировано `tools_step::run_tool_calls`, телеметрия — `trace::StepTrace` | ~720 |
| `src/agent/tools_step.rs` | Вызовы инструментов одного шага: `run_tool_calls` (лимит `max_tools_per_step` с явным «Skipped:» `tool_result`), `execute_tool_call` → `ask_the_user`/`spawn_task`/`run_generic_tool` (все возвращают `ToolOutcome{text,is_error}`, а не кортеж), `ToolExecContext` + `emit`/`emit_tool_started`, `dispatch_generic_tool` (короткий лок MCP), `tool_skip_message`, `summarize_tool_result` (режет общим `util::truncate_at_char_boundary`). Вынесено из `runner.rs` 2026-08-27 | ~300 |
| `src/agent/trace.rs` | `StepTrace` — фасад над `debug_log` для телеметрии шага. Строки байт-в-байт те же, что раньше писались в `runner.rs` вручную (инвариант 11); общий префикс `conversation=N step=M/K` был скопирован 18 раз, теперь собирается один раз в `new`. Парные записи (строка + `.payload`) пишутся одним методом: `parsed`, `tool_call`, `tool_result(&ToolResultTrace)` | ~200 |
| `src/agent/retry.rs` | `RetryBudget` — два независимых счётчика повторов (`take_malformed`/`take_empty`) вместо ручных `u32` в обоих циклах; сюда же переехали `MAX_MALFORMED_TOOL_RETRIES`/`MAX_EMPTY_RESPONSE_RETRIES` | ~60 |
| `src/agent/stream.rs` | `collect_stream(provider, request, on_progress) -> StreamOutcome{text, got_stop, end: StreamEnd}` — the once-duplicated spawn-stream + 120s-idle-guard + post-mortem block shared by runner and sub_agent (extracted 2026-07-04). `StreamVerdict` + `StreamOutcome::verdict()` (2026-08-27) — единый разбор пары `end`+`got_stop`, который оба цикла раскладывали по-своему. Reports what happened; callers interpret (runner treats Completed-without-stop as error w/ salvage, sub_agent doesn't) | ~210 |
| `src/agent/sub_agent.rs` | `run_sub_agent(SubAgentSpec)` — безголовый изолированный прогон (свой форк провайдера, авто-аппрув), стрим с пустым колбэком, возвращает финальный текст. С 2026-08-27 разбирает конец стрима общим `StreamOutcome::verdict()` и повторяет пустой ответ через `RetryBudget` — раньше отдавал пустоту родителю как успех; на исчерпании шагов берёт последний **непустой** текст, иначе ошибка | ~170 |
...
| `src/commands/defs/*.rs` | 42 files, 48 registry entries (count corrected 2026-08-26 — a few files register a second/third related command, e.g. `cwd`, `agent`, `chats`) — incl. `/btw`, `/new`+`/chats`, `/agent`+`/agents`, `/goal` (leading-slash registration bug fixed), `/logout`, `/wipe` — see `reference/COMMANDS.md`. `/help` is now generated from the live registry instead of a hand-maintained list. |
| `src/commands/defs/compact.rs` | `/compact [1|2|3]` (2026-08-26, replacing the stub) — parses the mode only and returns `CommandResult::Compact`; a synchronous command cannot make a model call, so `App::spawn_compaction` (`src/app/multichat.rs`) does the work off-loop and answers with `AppEvent::CompactFinished` |
| `src/commands/defs/default_compact.rs` | `/default-compact [1|2|3]` (2026-08-26) — persists `[context] compact_mode`, the fallback for chats that have not set `Conversation.compact_mode`; with no argument it reports the current default and lists the modes |
...
| File | Purpose | Lines |
|------|---------|-------|
| `src/harness/mod.rs` | clap subcommands (`exec`/`scenario`/`suite`/`mine`/`mock-provider`) + `run()` dispatch returning a process exit code; `GlobalFlags` — глобальные `--config`/`--data-dir`, которые наследуют дочерние `exec`; `parse_semantic` (`off`/`background`/`ready[:s]`), `run_stamp`, `DEFAULT_OUT_DIR` (`.dev/harness`) | ~300 |
| `src/harness/driver.rs` | One real turn without a terminal: `assemble` (same deps as `App::new`) → `AgentRuntime::spawn(TurnSpec)` → `drive` (services `AppEvent`s under a wall-clock deadline). `--resume <session-id>` продолжает сохранённую сессию (история + серверная сессия провайдера, дозапись **в тот же** файл; неизвестный id — `setup_failed`). `ApprovePolicy` answers approvals (**`auto_approve` stays false** — `run_agent_loop` refuses sub-agents when it is on), `SemanticMode` gates RAG readiness, `SpawnSubAgent` honoured for real, `absolutize` keeps the trace out of the workspace under test. `RunOutcome`/`RunStatus` (exit 0/1/2/3). С 2026-08-27 историю корневой беседы ведёт общий `app::reduce::apply`, а не собственные плечи `match` — своя копия `apply_cleared_tool_output` удалена | ~1040 |
...
| `src/harness/scenario.rs` | `Scenario` + `Expect` TOML (`deny_unknown_fields` on both — a mistyped expectation must not silently pass), `Expect::check` → failure reasons, `execute` fans repeats out as **child processes** (`exec --json`, `kill_on_drop` + timeout+60s backstop, `GlobalFlags` (`--config`/`--data-dir`) forwarded), `plant_session` сажает копию фикстуры `session_template` под свежим id на каждый повтор, `collect_scenarios`, `EXIT_EXPECTATIONS_FAILED = 4` | ~530 |
...
| `src/context/mod.rs` | Context-compaction ladder, peer of `semantic/` — module doc points at `.docs/context-compaction.md`; re-exports `budget::{ContextBudget, budget_tokens, conversation_tokens}`, `spec::ContextSpec`, `tool_output::cap_tool_output`; `PRUNE_TRIGGER_PERCENT` (70) gates rung 1, `SESSION_RESET_PERCENT` (90) gates rung 2; `modes`/`summary`/`compact` are rung 3 |
...
| `src/context/prune.rs` | Step 3 / ladder rung 1 (2026-08-26): `plan_prune(messages, protect_tokens)` — oldest-first tool-result victims, walking backwards from the in-flight tail (`in_flight_tail_start`, protected unconditionally); `freed_tokens`, `cleared_marker`/`is_cleared`, `spill_file_name` (sanitises a model-supplied tool-call id to `[A-Za-z0-9_-]`). Pure — `agent/runner.rs::prune_tool_output` does the spilling/rewriting and is skipped outright when `LLMProvider::keeps_server_side_history()` is true (DeepSeek) |
| `src/context/modes.rs` | Step 5 / ladder rung 3, where to cut (2026-08-26): `CompactMode` (1 first+last verbatim, 2 chunked oldest half, 3 all but the last turn; `from_number` falls back to 1) and `plan()` → `CompactPlan { head, summarize, tail }`. Pure index maths — turn boundaries from `turn_starts` (a turn owns its tool calls, so a cut never splits call from result), `MODE2_CHUNK_TOKENS` (2 000, chunk *size* not chunk count), `MODE1_HEAD_SHARE` (the opening turn is kept only under a quarter of the usable window). Fewer than two turns → empty plan |
| `src/context/summary.rs` | Step 5 / ladder rung 3, what to ask (2026-08-26): `build_request` (opencode-style `<template>`, five sections with `(none)`, plus merge rules when a `<prior-summary>` exists), `validate` (every heading or the reply is refused — a truncated form must not become the whole context), `observed_facts` (`## Tools Used` computed from tool messages, §5.4 — never asked of the model). `TOOL_RESULT_CHARS_FOR_SUMMARY` = 2 000 cuts tool bodies before the summariser. A test asserts the prompt never asks for a compression ratio |
| `src/context/compact.rs` | Step 5 / ladder rung 3, the executor (2026-08-26): `compact(provider, messages, mode, usable, model)` → `CompactOutcome { messages, calls, before_tokens, after_tokens }`. One model call per chunk at `SUMMARY_MAX_TOKENS` = 4 096 / `temperature 0.2`; summary re-enters as a **user** message prefixed `SUMMARY_PREFIX` (`[Context summary]`), which the next run detects and merges instead of summarising a summary (first chunk only). Refuses and keeps the original history when the reply is short of a section or the summary is not smaller. Knows nothing of `App` (invariant 6) |
| `src/context/spec.rs` | `ContextSpec` — the per-turn ladder settings (`auto_compact`, window, `preserve_recent_tokens`, spill dir under `data_dir/tool-output/<session>/`), carried on `TurnSpec.context` from every call site (TUI, sidechat, harness). `Default` keeps the ladder off |
| `src/tools/read_file.rs` | `read_file` builtin (2026-08-26) — paged UTF-8 read (1-based `offset`/`limit`, default 400 lines), UTF-16 decoded via `util::decode_process_output`, blocking read on `spawn_blocking`. The model's way to fetch a rung-1-cleared tool body back from disk |
...
| `scripts/release.sh` | `patch\|minor\|major\|X.Y.Z [--dry-run] [--yes]` — bumps `Cargo.toml`, tags `vX.Y.Z`, pushes the tag to trigger `release.yml` |
| `scripts/install.sh` | `curl \| sh` installer for macOS arm64 / Linux — `~/.local/bin`, `--dir`, `--channel`, `--uninstall` |
| `scripts/ci/package.sh`, `scripts/ci/make-manifest.sh`, `scripts/ci/collect-assets.sh`, `scripts/ci/changelog-section.sh` | CI helpers shared by `build.yml`/`release.yml`: per-target packaging, `manifest.json` generation (version/tag/commit/asset hashes), asset collection, changelog-section extraction |
| `scripts/render-release-notes.sh` | Подстановка `{{VAR}}` в шаблон заметок к выпуску; общий для `ci.yml` (`publish-dev`), `release.yml` и `.gitlab-ci.yml`, чтобы все конвейеры давали одинаковый текст |
| `scripts/dev-release.template.md` | Шаблон заметок катящегося `dev`-релиза (used by `ci.yml`'s `publish-dev` job) |
| `scripts/stable-release.template.md` | Шаблон заметок стабильного релиза (used by `release.yml`) |
...
- Conversations: `App.state.conversations: Conversations` (`app/conversation.rs`) — one focused, others background; агентские события идут через `on_agent_event` (`app/mod.rs`) → `reduce::apply` → `reduce::turn_tail` (`app/reduce.rs`). Развилка по **виду** беседы, не по фокусу, поэтому побочный чат сворачивается в родителя даже будучи на экране
